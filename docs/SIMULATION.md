...
- `test_runner_foc_readiness_gate`：真实 ADC injected callback 在初始化完成前不得触碰 ADC 更新、编码器、安全、控制器、observer、PWM 或心跳路径；ready 后才进入该运行路径。
...
主机侧已覆盖确定性 ADC 噪声、ADC 样本保护、编码器失败守卫、看门狗监督、快速故障上报/回调重试、故障清除状态契约和通信命令边界。

下列场景仍需要硬件在环（HIL）或台架验证，不能声称已经由主机仿真覆盖：

- 三相缺相、栅极/PWM 卡死、实际过流和母线浪涌；
- 编码器断线、冻结、跳变和磁干扰；
- 真实 ADC 饱和、采样时序漂移、DMA/中断延迟；
- CAN 总线拥塞、仲裁丢失、物理 TX abort 和 USB CDC 断连/重连；
- 真实负载堵转、热模型、低压启动和掉电恢复；
- 运行时栈高水位、CCM/RAM 余量和看门狗复位恢复。固件现已在 `task_guard` 的每秒诊断日志中输出 `stack_free_w=default/guard/comm`（单位为 FreeRTOS word），但尚未在真实负载下采集这些数据。
