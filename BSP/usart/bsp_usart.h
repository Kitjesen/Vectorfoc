 ...
 * @brief 串口实例结构体
 */
#define USART_TXBUFF_SIZE 2048 // 发送缓冲区大小

typedef struct {
  uint8_t recv_buff[USART_RXBUFF_LIMIT]; // 预先定义的最大buff大小
  ...
  volatile uint8_t is_transmitting; // 1: DMA发送中
  volatile uint16_t last_tx_len;    // 上一次DMA传输的长度
} USARTInstance;
