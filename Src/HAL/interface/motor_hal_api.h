 ...
 * allowing it to be decoupled from specific hardware drivers (STM32 HAL,
 * MT6816, etc.).
 */
...
typedef struct {
  float position_rad; // Multi-turn mechanical position [rad], relative to boot
                      // zero
  float angle_rad;    // Single-turn mechanical angle [rad] (0 ~ 2PI)
  float velocity_rad; // Mechanical velocity [rad/s]
  float elec_angle;   // Electrical angle [rad] (-PI ~ PI) (Optional, or
                      // calculated by core)
...
typedef struct {
  ...
  /**
   * @brief Start the ADC trigger timer while bridge outputs remain disabled.
   * @return true when sampling is active.
   */
  bool (*start_sampling)(void);
